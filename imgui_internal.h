...
#define IM_F32_TO_INT_UNBOUND(_VAL, _MAX) ((int)((_VAL) * static_cast<float>(_MAX) + ((_VAL)>=0 ? 0.5f : -0.5f)))   // Unsaturated, for display purpose
...
enum ImGuiNextWindowDataFlags_
{
    ...
    ImGuiNextWindowDataFlags_BringToFront       = 1 << 14,
};
...
struct ImGuiContext
{
    ...
    float title_bar_height_bias = 0.f; // HACK(JF) To fix the computation of Window::TitleBarHeight() when using a different font / padding for the window titles than for the rest of the UI.
...
//#endif
    ...
    IMGUI_API void          RenderFrame(ImVec2 p_min, ImVec2 p_max, ImU32 fill_col, bool borders = true, float rounding = 0.0f, ImDrawFlags draw_flags = 0);
    ...
    IMGUI_API void          RenderArrowDockMenu(ImDrawList* draw_list, ImVec2 p_center, float sz, ImU32 col);
    ...
    IMGUI_API float         CloseButtonSize();
    ...
    IMGUI_API bool          DataTypeApplyFromText(const char* buf, ImGuiDataType data_type, void* p_data, const char* format, void* p_data_when_empty = NULL, CustomParser custom_parser = nullptr);
